...
class Three(multiprocessing.Process if __MULTIPROCESSING__ else threading.Thread):
    ...
    def run(self):
        try:
            self.logger.info("Running Three process")
...
def test():
    ...
    try:
        pTest = Three(logger, logconfig, qOne, qTwo, qThr)
        pTest.start()
